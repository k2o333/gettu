...
def build_with_date_range(data_types=None, start_date='20050101', end_date=None):
    ...
    for data_type in types_to_build:
        ...
        # 检查是否已有数据，如果有则从最后更新日期之后开始下载
        last_update = get_last_update_date(data_type)
        actual_start_date = start_date

        if last_update:
            # 如果已有数据，从最后更新日期的下一天开始
            # 将日期格式从 YYYYMMDD 转换为datetime对象，加一天后再转回来
            ...
            if actual_start_date > end_date:
                logging.info(f"{data_type} 数据已更新到 {last_update}，跳过构建")
                continue

            logging.info(f"{data_type} 已有数据更新到 {last_update}，从 {actual_start_date} 继续下载...")
        else:
        ...
        try:
            ...
            if strategy == 'by_stock':
                _build_by_stock_with_date_range(data_type, config, actual_start_date, end_date)
            elif strategy == 'by_date':
                _build_by_date_range(data_type, config, actual_start_date, end_date)
            elif strategy == 'by_period':
                _build_by_period_range(data_type, config, actual_start_date, end_date)
            elif strategy == 'by_exchange':
                _build_by_exchange_with_date_range(data_type, config, actual_start_date, end_date)
            else:
                # 默认策略：尝试直接下载指定日期范围的数据
                _build_default_with_date_range(data_type, config, actual_start_date, end_date)
...
def build_with_enhanced_scan(data_types=None, start_date='20050101', end_date=None):
    ...
    from enhanced_scanner import build_with_enhanced_scan as enhanced_build
    enhanced_build(data_types, start_date, end_date)